 ...
 *   node scripts/vendor-skills.mjs --keep-temp     # inspect the scratch dir
 *
...
function parseArgs(argv) {
  const options = { version: 'latest', keepTemp: false };
 ...
 */
async function renderSkills(version) {
  ...
  try {
    ...
    for (const name of skills) {
      await cp(join(generatedRoot, name), join(SKILLS_DIR, name), { recursive: true });
    }
    ...
    const sample = await readFile(join(SKILLS_DIR, skills[0], 'SKILL.md'), 'utf8');
