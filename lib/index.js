...
export async function installShim() {
    ...
    for (const directory of installDirPreference()) {
        ...
        if (resolved.kind === 'installed' && resolved.path === path)
...
export async function removeShim() {
    ...
    if (state.kind !== 'installed')
        return { ok: false, reason: state.kind, path: state.kind === 'foreign' ? state.path : undefined };
...
async function diagnose() {
    ...
    const launcher = shim.kind === 'absent' ? 'absent' : `${shim.kind} (${shim.path})`;
    const lines = [
        'dsh-openspec',
    ...
    if (installedVersion === undefined) {
        ...
        lines.push('', `\`openspec\` resolves to ${shim.path}, which dsh-openspec did not write.`, 'The skills will call that binary. If it is an OpenSpec version whose output', `differs from this plugin's vendored skills (${installedVersion}), install the`, 'launcher into a directory earlier on PATH, or remove the other install and', 'run /openspec shim.');
    }
    return lines.join('\n');
}
/** Whether a shell resolves `openspec` at all, whoever provides it. */
async function openspecResolves() {
...
export function apply(ctx) {
    ...
    void (async () => {
        ...
        // false alarm.
        if (await openspecResolves())
            return;
        const outcome = await installShim();
        if (outcome.ok)
            ctx.logger.info(`${name}: installed the \`openspec\` launcher at ${outcome.path} (remove with \`/openspec uninstall-shim\`).`);
        else
            ctx.logger.warn(`${name}: \`openspec\` does not resolve and the launcher could not be installed (${outcome.reason}). ${outcome.message}`);
    })().catch((error) => {
